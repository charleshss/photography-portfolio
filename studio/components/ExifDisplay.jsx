                ...
                .then((asset) => {
                    ...
                    if (exif && !initialized) {
                        setEditableData({
                            ...
                            captureDate:
                                currentCameraData.captureDate ||
                                formatDate(exif.DateTimeOriginal || exif.DateTime) ||
                                '',
                            aperture:
                                currentCameraData.settings?.aperture ||
                                (exif.FNumber || exif.ApertureValue
                                    ? `f/${exif.FNumber || exif.ApertureValue}`
                                    : '') ||
                                '',
                            shutterSpeed:
                                currentCameraData.settings?.shutterSpeed ||
                                (exif.ExposureTime
                                    ? exif.ExposureTime >= 1
                                        ? `${exif.ExposureTime}s`
                                ...
                                '',
                            iso:
                                currentCameraData.settings?.iso ||
                                exif.ISO ||
                                exif.ISOSpeedRatings ||
                                '',
                            focalLength:
                                currentCameraData.settings?.focalLength ||
    ...
    const handleSave = () => {
        ...
        const cameraDataToSave = {
            ...
            settings: {
                ...
                iso: editableData.iso
                    ? parseInt(editableData.iso.toString().replace(/\D/g, ''))
                    : null,
                focalLength: editableData.focalLength,
            },
        }

        props.onChange(PatchEvent.from([set(cameraDataToSave, ['cameraData'])]))
        setIsEditing(false)
    }
            ...
            setEditableData({
                camera:
                    ...
                    (exifData.Make && exifData.Model
                        ? `${exifData.Make} ${exifData.Model}`
                        : exifData.Model || exifData.Make) ||
                    '',
                lens: currentCameraData.lens || exifData.LensModel || exifData.LensMake || '',
                captureDate:
                    currentCameraData.captureDate ||
                    formatDate(exifData.DateTimeOriginal || exifData.DateTime) ||
                    '',
                aperture:
                    currentCameraData.settings?.aperture ||
                    (exifData.FNumber || exifData.ApertureValue
                        ? `f/${exifData.FNumber || exifData.ApertureValue}`
                        : '') ||
                    '',
                shutterSpeed:
                    currentCameraData.settings?.shutterSpeed ||
        ...
        return (
            <Card padding={3} tone="caution" border>
                <Stack space={2}>
                    ...
                    <Text size={1} muted>
                        Upload an image with embedded camera data to see automatic EXIF information
                        here.
                    </Text>
                </Stack>
            </Card>
        ...
        <Card padding={3} tone="positive" border>
            <Stack space={4}>
                ...
                {fields.length > 0 ? (
                    ...
                    <Text size={1} muted>
                        No camera data found in this image's EXIF information.
                    </Text>
                )}

                <Text size={0} muted>
                    💡 Data automatically extracted from your uploaded image. Click "Edit" to make
